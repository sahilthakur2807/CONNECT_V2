...
function validateSetting(key: string, value: string, expectedType: string): boolean {
...
async function main() {
  ...
  for (const s of REQUIRED_SETTINGS) {
    if (!validateSetting(s.key, s.value, s.type)) {
      throw new Error(`Validation failed for seeded setting: key="${s.key}", value="${s.value}", type="${s.type}"`);
    }
