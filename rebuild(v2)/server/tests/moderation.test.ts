...
import { CreateReportHandler, CreateReportCommand, AssignReportHandler, AssignReportCommand, ResolveReportHandler, ResolveReportCommand, ExecuteModerationActionHandler, ExecuteModerationActionCommand, ResolveAppealHandler, ResolveAppealCommand, RemoveContentCommand, RemoveContentHandler, RestoreContentCommand, RestoreContentHandler } from '../src/features/moderation/application/commands/ModerationCommands.js';
import { GetReportsHandler, GetReportsQuery, GetAuditLogsHandler, GetAuditLogsQuery } from '../src/features/moderation/application/queries/ModerationQueries.js';
...
describe('CONNECT Phase 6 Moderation & Governance Unit Tests', () => {
  ...
  describe('ModerationPolicy Authorizations', () => {
    it('should deny platform bans to regular users', () => {
      const allowed = ModerationPolicy.canExecutePlatformAction({ id: 'usr_member', role: 'user' });
      expect(allowed).toBe(false);
    });

    it('should allow platform bans to platform admins', () => {
      const allowed = ModerationPolicy.canExecutePlatformAction({ id: 'usr_admin', role: 'admin' });
      expect(allowed).toBe(true);
    });

    it('should allow community moderation to community moderators', () => {
      const membership = { id: 'mem_1', role: 'moderator', banned: false, muted: false };
      const allowed = ModerationPolicy.canExecuteCommunityAction({ id: 'usr_mod', role: 'user' }, membership);
      expect(allowed).toBe(true);
    });
  });
  ...
  describe('CreateReportHandler', () => {
    it('should create pending report and emit alert to moderators channel', async () => {
      ...
      const command = new CreateReportCommand('usr_reporter', 'spam', 'Spamming channels', 'usr_target');

      mockReportRepo.create.mockResolvedValue({ id: 'rep_123', status: 'pending' });
      ...
      expect(io?.emit).toHaveBeenCalledWith('report.created', expect.any(Object));
    });
  });
  ...
  describe('ExecuteModerationActionHandler', () => {
    it('should create enforcement action, update user status to offline, and log to audit repo', async () => {
      const handler = new ExecuteModerationActionHandler(mockActionRepo, mockAuditRepo, mockMembershipRepo);
    ...
    it('should check community memberships when executing community-scoped mute/bans', async () => {
      ...
      mockMembershipRepo.findMember.mockResolvedValue({ id: 'mem_1', role: 'moderator', banned: false, muted: false });
      mockActionRepo.create.mockResolvedValue({ id: 'act_123', type: 'mute' });

      const result = await handler.execute(command);

      expect(result.type).toBe('mute');
      expect(mockMembershipRepo.findMember).toHaveBeenCalledWith('usr_actor', 'comm_123');
      expect(mockActionRepo.create).toHaveBeenCalled();
    });
  });
  ...
  describe('ResolveAppealHandler', () => {
    it('should resolve appeal status and deactivate associated moderation action if approved', async () => {
      const handler = new ResolveAppealHandler(mockAppealRepo, mockActionRepo, mockAuditRepo);
      const command = new ResolveAppealCommand('usr_admin', 'admin', 'app_123', 'approved', 'Apologies accepted');

      mockAppealRepo.findById.mockResolvedValue({ id: 'app_123', actionId: 'act_123', status: 'pending' });
      mockAppealRepo.update.mockResolvedValue({ id: 'app_123', status: 'approved' });

      const result = await handler.execute(command);

      expect(result.status).toBe('approved');
      expect(mockActionRepo.update).toHaveBeenCalledWith('act_123', { active: false }, expect.any(Object));
      expect(mockAuditRepo.create).toHaveBeenCalled();
    });
  });
  ...
  describe('AuditLogRepository Immutability', () => {
    it('should throw ForbiddenError on update calls', async () => {
      ...
      await expect(repo.update('log_123', { action: 'hacked' })).rejects.toThrow(ForbiddenError);
    });
  ...
  describe('Content Moderation Handler', () => {
    it('should allow admins to remove messages administratively and write audit logs', async () => {
      ...
      const command = new RemoveContentCommand('usr_admin', 'admin', 'message', 'msg_123', 'Inappropriate content');

      mockMessageRepo.findById.mockResolvedValue({ id: 'msg_123', roomId: 'rm_123', deleted: false });
      mockRoomRepo.findById.mockResolvedValue({ id: 'rm_123', communityId: null }); // Global room

      await handler.execute(command);

      expect(mockMessageRepo.update).toHaveBeenCalledWith('msg_123', { deleted: true }, expect.any(Object));
    ...
    it('should allow admins to restore messages administratively and write audit logs', async () => {
      ...
      const command = new RestoreContentCommand('usr_admin', 'admin', 'message', 'msg_123', 'Accidental deletion');

      mockMessageRepo.findById.mockResolvedValue({ id: 'msg_123', roomId: 'rm_123', deleted: true });
      mockRoomRepo.findById.mockResolvedValue({ id: 'rm_123', communityId: null });

      await handler.execute(command);

      expect(mockMessageRepo.update).toHaveBeenCalledWith('msg_123', { deleted: false }, expect.any(Object));
      expect(mockAuditRepo.create).toHaveBeenCalledWith(expect.objectContaining({
