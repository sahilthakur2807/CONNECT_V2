...
import { AwardReputationHandler, AwardReputationCommand } from '../src/features/analytics/application/commands/AnalyticsCommands.js';
import { GetUserStatsHandler, GetUserStatsQuery, GetPlatformMetricsHandler, GetPlatformMetricsQuery } from '../src/features/analytics/application/queries/AnalyticsQueries.js';
...
// Mock repositories
vi.mock('../src/features/analytics/infrastructure/repository/ActivityFeedRepository.js', () => ({
  activityFeedRepository: {
    create: vi.fn()
  }
}));

vi.mock('../src/features/analytics/infrastructure/repository/ReputationLogRepository.js', () => ({
  reputationLogRepository: {
    logAward: vi.fn()
  }
}));

vi.mock('../src/features/analytics/infrastructure/repository/AnalyticsRepository.js', () => ({
  analyticsRepository: {
    findUserStats: vi.fn(),
...
describe('CONNECT Phase 8 Analytics & Reputation Unit Tests', () => {
  ...
  describe('GetUserStatsHandler', () => {
    it('should retrieve user statistics metrics', async () => {
      ...
      expect(analyticsRepository.findUserStats).toHaveBeenCalledWith('usr_target');
