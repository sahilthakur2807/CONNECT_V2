...
import { RegisterHandler, RegisterCommand } from '../src/features/auth/application/commands/RegisterHandler.js';
import { LoginHandler, LoginCommand } from '../src/features/auth/application/commands/LoginHandler.js';
import { RefreshTokenHandler, RefreshTokenCommand } from '../src/features/auth/application/commands/RefreshTokenHandler.js';
import { VerifyEmailHandler, VerifyEmailCommand } from '../src/features/auth/application/commands/VerifyEmailHandler.js';
import { RequestPasswordResetHandler, ResetPasswordHandler, RequestPasswordResetCommand, ResetPasswordCommand } from '../src/features/auth/application/commands/PasswordResetHandler.js';
import { RevokeSessionHandler, RevokeSessionCommand } from '../src/features/auth/application/commands/RevokeSessionHandler.js';
import { OAuthSignInHandler, OAuthSignInCommand } from '../src/features/auth/application/commands/OAuthSignInHandler.js';

import { AuthService } from '../src/features/auth/application/AuthService.js';
import { Hash } from '../src/shared/utils/Hash.js';
import { UnauthorizedError, BadRequestError } from '../src/shared/errors/AppError.js';
...
describe('CONNECT IAM Subsystem Unit Tests', () => {
  ...
  it('should register a new user successfully when inputs are valid', async () => {
    ...
    const command = new RegisterCommand('johndoe', 'john@example.com', 'password123');
  ...
  it('should fail registration if username is in reserved list', async () => {
    const handler = new RegisterHandler(mockUserRepo, authService);
    const command = new RegisterCommand('admin', 'admin@example.com', 'password123');
  ...
  it('should trigger lockout after 5 failed login attempts', async () => {
    ...
    expect(mockUserRepo.update).toHaveBeenCalledWith('usr_1', expect.objectContaining({
  ...
  it('should rotate session successfully with valid token', async () => {
    ...
    expect(mockSessionRepo.revokeSession).toHaveBeenCalledWith('sess_123', undefined);
  ...
  it('should revoke all sessions if a revoked refresh token is replayed', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow(UnauthorizedError);
    expect(mockSessionRepo.revokeAllForUser).toHaveBeenCalledWith('usr_1', undefined);
  });
  ...
  it('should reset password and revoke all active sessions', async () => {
    ...
    const command = new ResetPasswordCommand('valid-reset-token', 'newPassword123');
  ...
  it('should authenticate via Google OAuth and link email if user exists', async () => {
    ...
    const command = new OAuthSignInCommand('google', 'mock-google-token:oauth@example.com');
    ...
    const result = await handler.execute(command);
    expect(result.tokens.accessToken).toBeDefined();
    expect(mockUserRepo.linkOAuthAccount).toHaveBeenCalledWith('usr_oauth', 'google', 'google-uid-oauth@example.com');
  });
});
