...
import { SendMessageHandler, SendMessageCommand, EditMessageHandler, EditMessageCommand, DeleteMessageHandler, DeleteMessageCommand, RestoreMessageHandler, RestoreMessageCommand } from '../src/features/message/application/commands/MessageCommands.js';
import { GetRoomMessagesHandler, GetRoomMessagesQuery } from '../src/features/message/application/queries/MessageQueries.js';
...
describe('CONNECT Phase 4 Messaging Unit Tests', () => {
  ...
  describe('SendMessageHandler', () => {
    it('should send a message and trigger Socket.IO broadcast', async () => {
      const handler = new SendMessageHandler(mockMessageRepo, mockRoomRepo, mockCommunityRepo, mockMembershipRepo);
      const command = new SendMessageCommand('usr_1', 'room_123', 'Hello world', 'client-uuid-123');

      mockRoomRepo.findById.mockResolvedValue({ id: 'room_123', communityId: null, deleted: false, archived: false });
      mockMessageRepo.findByClientMessageId.mockResolvedValue(null);
      mockMessageRepo.create.mockResolvedValue({ id: 'msg_1', content: 'Hello world' });
      mockMessageRepo.findById.mockResolvedValue({ id: 'msg_1', content: 'Hello world', userId: 'usr_1', roomId: 'room_123' });
      ...
      expect(io?.emit).toHaveBeenCalledWith('chat.message.created', expect.objectContaining({
        success: true,
  ...
  describe('Edit & Delete Handlers', () => {
    it('should edit message and broadcast event', async () => {
      ...
      const command = new EditMessageCommand('usr_author', 'msg_1', 'Edited text');

      mockMessageRepo.findById.mockResolvedValue({ id: 'msg_1', userId: 'usr_author', roomId: 'room_123', deleted: false });
      mockMessageRepo.update.mockResolvedValue({ id: 'msg_1', content: 'Edited text', edited: true });

      const result = await handler.execute(command);
      expect(result.edited).toBe(true);
      expect(io?.emit).toHaveBeenCalledWith('chat.message.updated', expect.any(Object));
    });
  });
  ...
  describe('GetRoomMessagesHandler cursor checks', () => {
    it('should trigger findHistoryBefore query when direction is before', async () => {
      ...
      const query = new GetRoomMessagesQuery('room_123', 20, 'cursor_id', 'before');

      await handler.execute(query);
      expect(mockMessageRepo.findHistoryBefore).toHaveBeenCalledWith('room_123', 20, 'cursor_id');
    });

    it('should trigger findHistoryAfter query when direction is after', async () => {
      const handler = new GetRoomMessagesHandler(mockMessageRepo);
      const query = new GetRoomMessagesQuery('room_123', 20, 'cursor_id', 'after');

      await handler.execute(query);
      expect(mockMessageRepo.findHistoryAfter).toHaveBeenCalledWith('room_123', 20, 'cursor_id');
    });
  });
  ...
  describe('RoomJoinHandler socket checking', () => {
    it('should deny joining room channel if user is not community member', async () => {
      ...
      mockRoomRepo.findById.mockResolvedValue({ id: 'room_1', communityId: 'comm_1' });
      ...
      expect(socket.emit).toHaveBeenCalledWith('chat.room.joined.response', expect.objectContaining({
        success: false,
