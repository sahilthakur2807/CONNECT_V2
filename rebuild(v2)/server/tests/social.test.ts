...
import { SendFriendRequestHandler, SendFriendRequestCommand, AcceptFriendRequestHandler, AcceptFriendRequestCommand, BlockUserHandler, BlockUserCommand } from '../src/features/social/application/commands/SocialCommands.js';
import { GetNotificationsHandler, GetNotificationsQuery } from '../src/features/social/application/queries/SocialQueries.js';
import { SocialPolicy } from '../src/features/social/application/SocialPolicy.js';
import { activeUserConnections, io } from '../src/infrastructure/socket/SocketServer.js';
...
describe('CONNECT Phase 5 Social & Presence Unit Tests', () => {
  ...
  describe('SocialPolicy validations', () => {
    it('should deny sending friend request to oneself', () => {
      const allowed = SocialPolicy.canRequestFriendship('usr_1', 'usr_1', false);
  ...
  describe('SendFriendRequestHandler', () => {
    it('should create pending friendship, insert notification, and emit socket alert', async () => {
      const handler = new SendFriendRequestHandler(mockFriendshipRepo, mockBlockRepo, mockNotificationRepo);
      const command = new SendFriendRequestCommand('usr_sender', 'usr_recipient');

      mockBlockRepo.hasBlockRelationship.mockResolvedValue(false);
      mockFriendshipRepo.findFriendship.mockResolvedValue(null);
      mockFriendshipRepo.create.mockResolvedValue({ id: 'fr_123', status: 'pending' });
      mockNotificationRepo.create.mockResolvedValue({ id: 'notif_1', type: 'friend.request.sent' });
      ...
      expect(io?.emit).toHaveBeenCalledWith('notification.created', expect.any(Object));
    });
  });
  ...
  describe('BlockUserHandler', () => {
    it('should create block record and delete active friendship between them', async () => {
      ...
      expect(mockFriendshipRepo.delete).toHaveBeenCalledWith('fr_123', expect.any(Object));
    });
  });
  ...
  describe('GetNotificationsHandler', () => {
    it('should query history with specific cursor', async () => {
      ...
      expect(mockNotificationRepo.findHistory).toHaveBeenCalledWith('usr_1', 15, 'notif_cursor');
    });
  });
