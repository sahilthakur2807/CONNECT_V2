...
} from "../src/features/discovery/application/queries/DiscoveryQueries.js";
...
describe("CONNECT Phase 7 Discovery & Search Unit Tests", () => {
  ...
  describe("SearchUsersHandler", () => {
    it("should invoke searchUsers on the search engine", async () => {
      ...
      mockSearchEngine.searchUsers.mockResolvedValue({
        items: [],
  ...
  describe("SearchRoomsHandler", () => {
    it("should invoke searchRooms on searchEngine passing userId", async () => {
      ...
      mockSearchEngine.searchRooms.mockResolvedValue({
        items: [],
        nextCursor: undefined,
      });
  ...
  describe("SearchMessagesHandler", () => {
    it("should fetch permitted room IDs first and then pass them to the search engine", async () => {
      ...
      mockSearchEngine.searchMessages.mockResolvedValue({
        items: [],
        nextCursor: undefined,
      });
      ...
      expect(prisma.room.findMany).toHaveBeenCalledWith({
        where: {
          deleted: false,
          OR: [{ communityId: null }, { communityId: { in: ["comm_1"] } }],
        },
        select: { id: true },
      });
  ...
  describe("GetTrendingContentHandler", () => {
    it("should pull trending communities and rooms from the discovery repo", async () => {
      ...
      mockDiscoveryRepo.findTrendingCommunities.mockResolvedValue([
        { id: "comm_trend_1" },
      ]);
      mockDiscoveryRepo.findTrendingRooms.mockResolvedValue([
        { id: "room_trend_1" },
      ]);
  ...
  describe("GetRecommendedCommunitiesHandler", () => {
    it("should request community recommendations for the user", async () => {
      ...
      mockDiscoveryRepo.findRecommendedCommunities.mockResolvedValue([
        { id: "comm_rec_1" },
      ]);

      const result = await handler.execute(query);

      expect(mockDiscoveryRepo.findRecommendedCommunities).toHaveBeenCalledWith(
        "usr_id_123",
        8,
      );
