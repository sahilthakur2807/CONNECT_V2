...
export class DiscoveryRepository {
   ...
   */
  async findTrendingCommunities(limit = 10): Promise<any[]> {
   ...
   * Score = (Members * 1.0) + (Messages in last 24h * 3.0)
   */
  async findTrendingRooms(limit = 10): Promise<any[]> {
    const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
   ...
   */
  async findRecommendedCommunities(userId: string, limit = 10): Promise<any[]> {
