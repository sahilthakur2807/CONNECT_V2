...
const getRecommendedCommunitiesHandler = new GetRecommendedCommunitiesHandler(discoveryRepo);

export function createDiscoveryRouter(): Router {
  const router = Router();

  // 1. Search Users
  router.get('/search/users', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
  ...
  router.get('/search/communities', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      q: z.string().default(''),
      limit: z.preprocess((val) => parseInt(val as string) || 20, z.number().min(1).max(100)),
      cursor: z.string().optional()
    ...
    try {
      const parsed = schema.parse(req.query);
      const query = new SearchCommunitiesQuery(parsed.q, parsed.limit, parsed.cursor);
  ...
  // 3. Search Rooms
  router.get('/search/rooms', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      q: z.string().default(''),
      limit: z.preprocess((val) => parseInt(val as string) || 20, z.number().min(1).max(100)),
      cursor: z.string().optional()
    ...
    try {
      const parsed = schema.parse(req.query);
      const query = new SearchRoomsQuery(req.user!.id, parsed.q, parsed.limit, parsed.cursor);
  ...
  // 4. Search Messages
  router.get('/search/messages', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      q: z.string().min(1),
      limit: z.preprocess((val) => parseInt(val as string) || 20, z.number().min(1).max(100)),
      cursor: z.string().optional()
    ...
    try {
      const parsed = schema.parse(req.query);
      const query = new SearchMessagesQuery(req.user!.id, parsed.q, parsed.limit, parsed.cursor);
  ...
  router.get('/discovery/trending', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      limit: z.preprocess((val) => parseInt(val as string) || 10, z.number().min(1).max(100))
    });
  ...
  router.get('/discovery/recommendations', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      limit: z.preprocess((val) => parseInt(val as string) || 10, z.number().min(1).max(100))
    });
    ...
    try {
      const parsed = schema.parse(req.query);
      const query = new GetRecommendedCommunitiesQuery(req.user!.id, parsed.limit);
