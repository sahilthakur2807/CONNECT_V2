...
// Handlers
import { SendMessageCommand, SendMessageHandler, EditMessageCommand, EditMessageHandler, DeleteMessageCommand, DeleteMessageHandler, RestoreMessageCommand, RestoreMessageHandler } from '../application/commands/MessageCommands.js';
import { GetRoomMessagesQuery, GetRoomMessagesHandler, GetMessageRepliesQuery, GetMessageRepliesHandler } from '../application/queries/MessageQueries.js';
...
const sendMessageHandler = new SendMessageHandler(messageRepo, roomRepo, communityRepo, membershipRepo);
...
export function createMessagesRouter(): Router {
  ...
  router.post('/rooms/:roomId/messages', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
  ...
  // 3. Edit message
  router.patch('/messages/:id', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    const schema = z.object({
      content: z.string().min(1).max(5000)
    });
  ...
  router.delete('/messages/:id', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    try {
  ...
  router.post('/messages/:id/restore', authenticateJWT, async (req: AuthenticatedRequest, res, next) => {
    try {
      const command = new RestoreMessageCommand(
        req.user!.id,
        req.params.id as string
      );
  ...
  router.get('/messages/:id/replies', async (req, res, next) => {
    try {
      const query = new GetMessageRepliesQuery(req.params.id as string);
