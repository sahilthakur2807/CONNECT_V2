...
export class AnalyticsRepository {
   ...
   */
  async findUserStats(userId: string): Promise<any> {
    ...
    const [messagesSent, communitiesJoined, roomsJoined, friends] = await Promise.all([
   ...
   */
  async findCommunityStats(communityId: string): Promise<any> {
    ...
    const [memberCount, activeMembers, roomCount, messagesSent24h] = await Promise.all([
   ...
   */
  async findPlatformMetrics(startDate: Date, endDate: Date): Promise<any> {
    const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);

    const [dau, registrations, messageVolume, moderationCount] = await Promise.all([
