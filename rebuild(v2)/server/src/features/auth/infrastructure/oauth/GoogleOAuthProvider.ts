import { type IOAuthProvider, type OAuthUserPayload } from './OAuthProvider.js';
...
export class GoogleOAuthProvider implements IOAuthProvider {
  ...
  async verifyToken(token: string): Promise<OAuthUserPayload> {
    ...
    if (process.env.NODE_ENV === 'test' && token.startsWith('mock-google-token')) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Google validation endpoint returned status: ${response.status}`);
      }

      const body = (await response.json()) as any;
      
      if (!body.sub || !body.email) {
        throw new Error('Google token info payload is missing identity coordinates (sub, email)');
      }
