...
// Handlers
import { RegisterCommand, RegisterHandler } from '../application/commands/RegisterHandler.js';
import { LoginCommand, LoginHandler } from '../application/commands/LoginHandler.js';
import { OAuthSignInCommand, OAuthSignInHandler } from '../application/commands/OAuthSignInHandler.js';
import { RefreshTokenCommand, RefreshTokenHandler } from '../application/commands/RefreshTokenHandler.js';
import { VerifyEmailCommand, VerifyEmailHandler } from '../application/commands/VerifyEmailHandler.js';
import { RequestPasswordResetCommand, ResetPasswordCommand, RequestPasswordResetHandler, ResetPasswordHandler } from '../application/commands/PasswordResetHandler.js';
import { RevokeSessionCommand, RevokeSessionHandler } from '../application/commands/RevokeSessionHandler.js';
...
export function createAuthRouter(): Router {
  ...
  router.post('/logout', optionalJWT, async (req: AuthenticatedRequest, res, next) => {
    const refreshToken = (req.cookies?.refreshToken) || req.body.refreshToken;
    const scope = (req.body.scope || 'current') as 'current' | 'all' | 'other';

    try {
      if (req.user) {
        const command = new RevokeSessionCommand(req.user.id, scope, refreshToken);
