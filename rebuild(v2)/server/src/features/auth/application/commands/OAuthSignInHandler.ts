...
export class OAuthSignInSuccessEvent implements IDomainEvent {
  readonly eventName = 'auth.oauth.success';
...
export class OAuthSignInHandler {
  ...
  async execute(command: OAuthSignInCommand): Promise<{ tokens: AuthTokens; user: any }> {
    const providerName = command.provider.toLowerCase();
    const provider = OAUTH_PROVIDERS[providerName];

    if (!provider) {
      throw new BadRequestError(`OAuth provider "${command.provider}" is not supported`);
    }
    ...
    let user = await this.userRepo.findByOAuth(providerName, oauthPayload.providerUserId);

    if (user) {
      // User exists, start session
      const tokens = await this.authService.createSession(user.id, command.deviceInfo, command.ipAddress);
      await EventBus.publish(new OAuthSignInSuccessEvent(user.id, providerName, command.ipAddress));
    ...
    if (existingUser) {
      ...
      await this.userRepo.linkOAuthAccount(existingUser.id, providerName, oauthPayload.providerUserId);
      
      const tokens = await this.authService.createSession(existingUser.id, command.deviceInfo, command.ipAddress);
      await EventBus.publish(new OAuthSignInSuccessEvent(existingUser.id, providerName, command.ipAddress));

      const { password: _, verificationToken: __, ...sanitizedUser } = existingUser;
      return { tokens, user: sanitizedUser };
    }

    // 4. Email does not exist, auto-signup new user
    // Generate unique username based on email prefix
    let baseUsername = sanitizedEmail.split('@')[0].replace(/[^a-zA-Z0-9]/g, '');
    ...
    const newUser = await this.userRepo.create({
      ...
      avatar: oauthPayload.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(username)}`,
      role: 'user',
      status: 'online',
    ...
    await this.userRepo.linkOAuthAccount(newUser.id, providerName, oauthPayload.providerUserId);

    const tokens = await this.authService.createSession(newUser.id, command.deviceInfo, command.ipAddress);
    await EventBus.publish(new OAuthSignInSuccessEvent(newUser.id, providerName, command.ipAddress));
