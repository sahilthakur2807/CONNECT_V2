...
import { initializeSocketServer, shutdownSocketServer } from './infrastructure/socket/SocketServer.js';
...
.then(() => {
  Logger.info('Successfully reset all stale online user statuses to offline.');
})
.catch(err => {
...
server.listen(config.PORT, '0.0.0.0', () => {
  Logger.info(`🚀 CONNECT Backend Server successfully booted on port ${config.PORT} [Environment: ${config.NODE_ENV}]`);
});
