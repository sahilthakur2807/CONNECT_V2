...
export function StatusDot({ status, className }: { status: 'online' | 'offline' | 'suspended' | string; className?: string }) {
  ...
  return (
    <span
      className={cn('inline-block w-2.5 h-2.5 rounded-full border-2 border-white dark:border-background', colors[statusKey], className)}
      aria-label={status}
    />
  );
}
