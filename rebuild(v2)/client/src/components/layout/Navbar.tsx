              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  ...
                  if (target.value.trim()) {
                    navigate(`/discover?q=${encodeURIComponent(target.value.trim())}`);
                  }
                }
              }}
              ...
              <DropdownMenuContent className="w-64 p-2 rounded-2xl shadow-2xl border-border/50">
                <DropdownMenuLabel className="p-3">
                  <div className="flex items-center gap-3">
                    <Avatar src={user.avatar || undefined} name={user.username} size="md" />
                ...
                <DropdownMenuItem onClick={() => { logout(); navigate('/auth'); }} className="text-red-600 focus:bg-red-50 focus:text-red-600 dark:focus:bg-red-950/30">
                  <LogOut size={16} />
          ...
          <div className="relative ml-auto flex h-full w-full max-w-xs flex-col bg-card py-4 shadow-xl animate-in slide-in-from-right duration-200">
            ...
            <div className="mt-auto p-6 border-t border-border bg-secondary/20">
              <div className="flex items-center gap-4 mb-6">
                <Avatar src={user.avatar || undefined} name={user.username} size="lg" />
                <div>
                  <p className="font-bold text-lg text-foreground truncate max-w-[150px]">{user.name || user.username}</p>
                  <p className="text-sm text-muted-foreground truncate max-w-[150px]">@{user.username}</p>
                </div>
              </div>
              <Button
                variant="destructive"
