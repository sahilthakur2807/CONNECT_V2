...
export function DiscussionRoom() {
  ...
  const { useRoomQuery, useTrendingRoomsQuery, joinRoomMutation, leaveRoomMutation } = useRooms();
  ...
  useEffect(() => {
    ...
    const handleActiveUsersUpdate = (data: { roomId: string; activeCount: number; activeUsers?: any[] }) => {
  ...
  if (roomLoading) {
    return (
      <div className="flex-grow flex flex-col justify-center items-center h-64 bg-background">
        ...
        <p className="mt-4 text-xs font-bold text-muted-foreground uppercase tracking-widest animate-pulse">Loading room details...</p>
      </div>
    );
  }

  if (!room) {
    return <div className="flex-grow flex justify-center items-center bg-background font-bold text-muted-foreground">Discussion room not found.</div>;
  }
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative bg-card">
        ...
        <header className="px-8 py-8 border-b border-border/50 bg-card/85 backdrop-blur-xl sticky top-0 z-20">
          <div className="flex items-start justify-between gap-8">
            <div className="min-w-0 space-y-3">
              ...
              <h1
                className="text-3xl md:text-4xl text-foreground tracking-tight leading-tight line-clamp-3 font-serif font-black"
              >
                {mainTitle}
              </h1>
          ...
          <div className="space-y-8 max-w-5xl mx-auto w-full">
            {messagesLoading ? (
              <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                <Activity className="animate-spin text-primary mb-4" size={28} />
                <span className="text-xs font-black uppercase tracking-widest animate-pulse">Retrieving Takes...</span>
              </div>
            ) : messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-muted-foreground text-center">
                <p className="text-sm font-semibold">No takes shared yet.</p>
                <p className="text-xs mt-1">Be the first to share your stance below!</p>
              </div>
            ) : (
              messages.map((msg) => (
        ...
        <div className="p-5 bg-card border-t border-border/50">
          <div className="max-w-4xl mx-auto space-y-4">
            {replyingTo && (
              <div
                className="flex items-center justify-between px-6 py-3 bg-primary/5 rounded-2xl border border-primary/10 overflow-hidden animate-in slide-in-from-bottom-2"
              >
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-primary/10 rounded-full cursor-pointer" onClick={() => setReplyingTo(null)}>
            ...
            <div className="flex items-center gap-2 p-1.5 bg-muted rounded-full border border-border/50 shadow-sm focus-within:border-primary/20 focus-within:shadow-md focus-within:bg-card transition-all">
              <div className="flex items-center gap-1 pl-2">
                <Button variant="ghost" size="icon" className="rounded-full h-10 w-10 text-muted-foreground hover:bg-secondary cursor-pointer">
                  <Smile size={20} />
                </Button>
                <Button variant="ghost" size="icon" className="rounded-full h-10 w-10 text-muted-foreground hover:bg-secondary cursor-pointer">
                  <Paperclip size={20} />
      ...
      <aside className="hidden xl:flex flex-col w-76 shrink-0 border-l border-border/50 bg-card p-8 space-y-10 overflow-y-auto">
        {isModeratorOrOwner && (
          <div className="p-2.5 bg-slate-900 text-white rounded-[24px] border border-border/20 space-y-3 relative overflow-hidden dark:bg-[#1a1a1a]">
            ...
            <div className="flex items-center gap-1">
              ...
              <h4 className="text-[10px] font-black text-primary uppercase tracking-widest">Moderator Directive</h4>
            </div>
        ...
        <div className="p-8 bg-muted rounded-[40px] border border-border/50 space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-black text-foreground uppercase tracking-wider font-sans">Discussion Pulse</h3>
          </div>
