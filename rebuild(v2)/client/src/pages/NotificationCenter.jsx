...
export function NotificationCenter() {
  ...
  const { useNotificationsQuery, markReadMutation, markAllReadMutation } =
    useNotifications();
  const {
  ...
  const handleRejectRequest = async (e, n) => {
    e.stopPropagation();
    if (!n.referenceId) return;
    try {
  ...
  const renderNotificationList = (filteredList) => {
        ...
        <div className="py-24 text-center space-y-6 bg-card rounded-[40px] border-2 border-dashed border-border animate-in fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mx-auto font-medium">
              Your inbox is empty. Why not start a debate or join a trending
              room?
            </p>
          </div>
          <Button
            ...
            variant="default"
            className="rounded-full px-8 h-12 font-black uppercase text-[10px] tracking-widest shadow-xl shadow-primary/20 cursor-pointer"
          >
