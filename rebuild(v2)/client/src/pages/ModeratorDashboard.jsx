    ...
    <div className="space-y-10 pb-10 max-w-7xl mx-auto font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in duration-300">
        {[
          ...
          <Card
            key={i}
            className="p-6 rounded-[32px] shadow-sm space-y-4 bg-card"
          >
            <div className="flex items-center justify-between">
              <div
                className={cn(
                  "p-2 rounded-xl bg-slate-50 dark:bg-muted",
                  stat.color,
                )}
              >
                <stat.icon size={20} />
              </div>
              <span className="text-[10px] font-black text-[#888880] uppercase tracking-widest">
                {stat.trend}
              </span>
            </div>
            <div>
              <p className="text-3xl font-black text-foreground font-serif">
                {stat.value}
              </p>
              <p className="text-[10px] font-bold text-[#888880] uppercase tracking-[0.2em] mt-1">
                {stat.label}
              </p>
            </div>
          </Card>
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-muted/50 border-b border-border h-16">
                <th className="font-black text-[10px] uppercase tracking-[0.2em] text-[#888880] pl-8">
                  Evidence & Source
                </th>
                <th className="font-black text-[10px] uppercase tracking-[0.2em] text-[#888880]">
                  Category
                </th>
                <th className="font-black text-[10px] uppercase tracking-[0.2em] text-[#888880]">
                  Heat Level
                ...
                <th className="font-black text-[10px] uppercase tracking-[0.2em] text-[#888880] text-right pr-8">
                  Control
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredReports.map((rep) => (
                <tr
                  key={rep.id}
                  className="border-b border-border/50 hover:bg-muted/30 transition-colors h-24"
                >
                  ...
                  <td>
                    <div
                      className={cn(
                        ...
                        rep.severity === "high"
                          ? "bg-red-50 text-red-600 border border-red-100 dark:bg-red-950/20"
                          : rep.severity === "medium"
                            ? "bg-orange-50 text-orange-600 border border-orange-100 dark:bg-orange-950/20"
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent className="w-56 rounded-2xl p-2 shadow-2xl border-border/50">
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              handleResolveReport(rep.id, "dismissed")
                            }
                          ...
                          {rep.reportedUserId && (
                            <DropdownMenuItem
                              onClick={() =>
                                openSuspendModal(
                                  rep.reportedUserId,
                                  rep.reportedUser?.username || "user",
                                )
                              }
          ...
          <div className="bg-card text-card-foreground rounded-[32px] max-w-lg w-full p-8 space-y-6 relative shadow-2xl border border-border/50">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Restrict user access controls on @{suspendUsername}.
              </p>
            </div>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label
                  htmlFor="enforce-type"
              ...
              {enforceType !== "warn" && (
                <div className="space-y-1.5 animate-in fade-in">
                  ...
                  <Input
                    ...
                    value={enforceDays}
                    onChange={(e) =>
                      setEnforceDays(parseInt(e.target.value) || 1)
                    }
                  />
                </div>
              )}
