...
import { MessageSquare, Award, Star, Zap, Activity, Calendar, ShieldAlert } from 'lucide-react';
...
export function UserProfile() {
  ...
  const { useFriendsQuery, usePendingRequestsQuery, blockUserMutation, sendFriendRequestMutation, acceptFriendRequestMutation } = useSocial();
  ...
  const { data: searchResults } = useSearchUsersQuery(resolvedUser?.username || 'a');
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      if (errMsg.includes('already exists') || err.response?.data?.message?.includes('already exists')) {
        setSentRequest(true);
  ...
  if (isLoading) {
    return (
      <div className="flex-grow flex flex-col justify-center items-center h-64 bg-background">
        ...
        <p className="mt-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Resolving Profile...</p>
      </div>
    );
  }
          ...
          <div className="flex justify-between items-start -mt-20 mb-4">
            ...
            {!isOwnProfile && (
              <div className="mt-24 flex gap-2">
                {isFriend ? (
                  <Button disabled className="rounded-2xl font-black uppercase text-[10px] tracking-widest h-10 px-5 bg-green-500/10 text-green-500 border border-green-500/20">
                    Friends
                  </Button>
                ) : pendingRequests.some((r) => r.user.id === targetId) ? (
                  <Button onClick={handleAcceptFriendDirect} className="rounded-2xl font-black uppercase text-[10px] tracking-widest h-10 px-5 bg-green-500 hover:bg-green-600 text-white cursor-pointer">
                    Accept Request
                  </Button>
                ) : sentRequest ? (
                  <Button disabled className="rounded-2xl font-black uppercase text-[10px] tracking-widest h-10 px-5 bg-muted text-muted-foreground border border-border/40">
                    Request Sent
                  </Button>
                ) : (
                  <Button onClick={handleAddFriend} className="rounded-2xl font-black uppercase text-[10px] tracking-widest h-10 px-5 cursor-pointer">
                    + Add Friend
                  </Button>
                )}
                <Button onClick={handleBlockUser} variant="outline" className="rounded-2xl font-black uppercase text-[10px] tracking-widest border-2 border-destructive/20 text-destructive hover:bg-destructive/10 h-10 px-5 cursor-pointer">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h2 
                  className="text-3xl text-foreground tracking-tight font-serif font-black"
                >
                  {profileUser.name || profileUser.username}
                </h2>
                {profileUser.verified && <Badge variant="verified" size="sm" showIcon={false} />}
              </div>
              <p 
            ...
            {profileUser.badges && profileUser.badges.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {profileUser.badges.map((b: string) => (
        ...
        <Card className="border-border/50 rounded-3xl shadow-sm bg-card hover:border-primary/20 transition-all">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h3 className="text-3xl font-black text-foreground">{stats?.roomsJoined || 0}</h3>
            </div>
        ...
        <Card className="border-border/50 rounded-3xl shadow-sm bg-card hover:border-amber-500/20 transition-all">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h3 className="text-3xl font-black text-amber-500">{profileUser.reputation || 0}</h3>
            </div>
        ...
        <Card className="border-border/50 rounded-3xl shadow-sm bg-card hover:border-blue-500/20 transition-all">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h3 className="text-3xl font-black text-blue-500">{(stats?.messagesSent || 0).toLocaleString()}</h3>
            </div>
        ...
        <Tabs defaultValue="activity" className="space-y-8">
          <div className="bg-card p-1.5 border border-border/50 rounded-2xl inline-flex shadow-sm">
            <TabsList className="bg-transparent border-none p-0 flex gap-1">
              <TabsTrigger value="activity" className="rounded-xl px-8 h-10 font-bold text-xs uppercase tracking-widest transition-all">
                Activity Feed
              </TabsTrigger>
              <TabsTrigger value="badges" className="rounded-xl px-8 h-10 font-bold text-xs uppercase tracking-widest transition-all">
                Badges & Honors
              </TabsTrigger>
            </TabsList>
          </div>
                ...
                >
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-foreground capitalize">{item.type.replace(/\./g, ' ')}</span>
          ...
          <TabsContent value="badges" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {profileUser.badges && profileUser.badges.length > 0 ? (
              profileUser.badges.map((badge: string) => (
                <div key={badge} className="p-8 bg-card border border-border/50 rounded-[32px] text-center space-y-4 hover:shadow-xl hover:shadow-primary/5 transition-all cursor-default animate-in fade-in">
                  <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto text-primary">
                    <Award size={32} />
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-bold text-foreground">{badge}</h4>
                    <p className="text-xs text-muted-foreground font-medium">Awarded for exceptional contributions to the network.</p>
                  </div>
                </div>
              ))
            ) : (
