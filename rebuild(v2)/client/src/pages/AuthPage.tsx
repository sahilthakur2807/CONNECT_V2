  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      if (!form.username || !form.email || !form.password) {
        ...
        setError('Username can only contain alphanumeric characters and underscores (no spaces).');
    ...
    <div className="flex min-h-screen bg-background overflow-hidden font-sans">
      ...
      >
        <div className="relative z-10 flex flex-col h-full">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 w-fit hover:opacity-80 transition-opacity cursor-pointer">
      ...
      <div className="flex-1 flex items-center justify-center p-8 sm:p-12 lg:p-16 overflow-y-auto bg-background">
        <div className="w-full max-w-[384px] animate-in fade-in duration-300">
          <div className="mb-6">
            <Link to="/" className="lg:hidden inline-flex items-center gap-2 mb-8 text-primary cursor-pointer">
               <div className="w-8 h-8 bg-primary flex items-center justify-center rounded-lg">
                <span className="text-primary-foreground font-serif text-sm">C</span>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {mode === 'register' && (
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[10px] text-muted-foreground leading-tight px-0.5">
                      Only letters, numbers, and underscores are allowed. No spaces. Min 3 characters.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder={mode === 'login' ? 'johndoe or name@example.com' : 'name@example.com'}
                    required
                    value={form.email}
                    onChange={handleChange}
                    className="h-[42px] px-3 rounded-[10px]"
