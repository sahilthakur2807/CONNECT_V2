...
export function LandingPage() {
  ...
  const [stats, setStats] = useState({ topics: 124, threads: 1482, citizens: 59 });
  ...
  useEffect(() => {
    ...
    apiClient.get('/admin/metrics?startDate=2020-01-01')
      .then((res) => {
        if (res.data?.success) {
          const data = res.data.data;
          setStats({
            topics: data.messageVolume ? Math.ceil(data.messageVolume / 4) : 124,
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-[#f0ede6] selection:bg-[#d42b2b]/30 selection:text-white flex flex-col font-sans overflow-hidden">
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled ? 'bg-[#0d0d0d]/95 backdrop-blur-sm border-b border-white/10 py-3' : 'py-5'
        }`}
      >
      ...
      <section className="relative min-h-[90vh] flex flex-col justify-center items-center px-6 pt-32 pb-20">
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 animate-in fade-in duration-1000">
            ...
            >
              Talk.<br />
              <span className="italic text-[#d42b2b]">Debate.</span><br />
              Discover.
            </h1>
            ...
            <div className="flex gap-10 border-t border-white/10 pt-8 max-w-md">
              {[
                ...
                <div key={s.label} className="space-y-1">
                  <span className="text-[#888880] block text-[9px] font-black uppercase tracking-widest">{s.label}</span>
                  <span className="text-2xl font-black text-white font-serif">{s.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative h-96 lg:h-[500px] w-full bg-[#161616] border border-white/10 rounded-[32px] overflow-hidden flex flex-col justify-between p-10 animate-in fade-in duration-1000">
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              <span className="text-[10px] text-[#888880] font-mono tracking-widest uppercase">DISCOURSE SIMULATION // CONNECT_V2</span>
              <Activity size={16} className="text-[#d42b2b]" />
            </div>

            <div className="space-y-6 font-serif">
              <div className="space-y-2">
                <span className="text-[10px] text-[#d42b2b] uppercase font-mono tracking-widest font-black">POLITICS & ETHICS SPHERE</span>
                <h3 className="text-xl md:text-2xl text-white font-black leading-tight">
                  "Does the proliferation of algorithmic regulation challenge democratic sovereignty?"
                </h3>
              </div>
              <p className="text-sm text-[#888880] leading-relaxed font-mono">
                Active debates involving 180 verified citizens with real-time reputation stakes and consensus branches.
              </p>
            </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-white/5">
              <div className="flex -space-x-2">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-8 h-8 rounded-full bg-[#333] border-2 border-[#161616]" />
                ))}
              </div>
              <span className="text-[10px] text-[#888880] font-mono">HEAT LEVEL: VERY HIGH</span>
            </div>
          </div>
        </div>
      </section>
