...
import { patchRoomStats } from '@/store/slices/roomSlice';
...
export function DiscussionRoom() {
  ...
  useEffect(() => {
    ...
    // Real-time: patch trending sidebar room counts + Redux currentRoom stats
    const handleRoomStats = (data: { roomId: string; memberCount?: number; messageCount?: number; activeNow?: number }) => {
      // Update Redux currentRoom (drives room header member/message count)
      dispatch(patchRoomStats(data));
      // Also update local trendingRooms sidebar list
  ...
  }, [roomId, openRoom, closeRoom, addMessage, updateMessageInList, removeMessageFromList, dispatch]);
              ...
              {otherTrending.map((r) => (
                <RoomCard
                  ...
                  room={{
                    ...
                    activeNow: r.activeNow ?? Math.ceil((r._count?.members || 1) * 0.4)
                  }}
                  compact
